...
const MiniStopDirectionList =({routeDirectionDatum,stopId, }:{routeDirectionDatum:RouteMatchDirectionInterface,stopId:string}) : JSX.Element | null =>{
...
const MiniStopDirectionListContainer = ({routeMatches,stopId,collapsed}:{routeMatches:RouteMatch[],stopId:AgencyAndId,collapsed:boolean}) : JSX.Element=>{
    return(
...
const RouteDirection = ({routeDirectionDatum,stopId, collapsed}:
    {routeDirectionDatum:RouteMatchDirectionInterface,stopId:string, collapsed:boolean}) : JSX.Element | null=>{
                ...
                <div className="card-content collapse-content">
                    <ul className="approaching-buses">
                        {vehicleData.map((vehicleDatum,index)=>{
                            if(index<3){return <VehicleComponent {...{vehicleDatum}} tabbable={0} key={index}/>}})}
                    </ul>
                    <ServiceAlertContainerComponent {...{abbreviatedRouteId: routeId, routeAgencyAndId: routeDirectionDatum.datumId.toString(), routeAndDirection: routeAndDir}} collapsed={!tabbable}/>
...
export function StopCardContent({stopMatch,collapsed}: { stopMatch: StopMatch, collapsed: boolean }):JSX.Element{
    ...
    return(
        <React.Fragment>
            ...
            {stopMatch.routeMatches.map((route: RouteMatch)=>
                route.directions.map((dir: RouteMatchDirectionInterface) => <RouteDirection key={uuidv4()} routeDirectionDatum={dir} stopId ={stopMatch.id.id} collapsed={collapsed}/>
                ))}
...
export function StopCard ({match}: {match: SearchMatch}) : JSX.Element {
        ...
        <div className="collapse-content">
            <div className="card-content px-2">
                ...
                <ul className="menu icon-menu card-menu">
                    <li>
                        <ViewSearchItem datumId={typeof stopMatch.id === 'string' ? stopMatch.id : stopMatch.id.id} text={"Stop"} collapsed={oneOfMany}/>
                    </li>
                </ul>
            </div>
...
export const StopCardWrapper = () : JSX.Element => {
    ...
    console.log("generating StopCardWrapper, match is: ", match)
    return (
    <>
        <h2 className="cards-header">Stops:</h2>
        <div className="cards">
            <StopCard match={match}/>
        </div>
    </>
)};
