...
export function StopCardFavoriteButton({stopMatch,collapsed}:{stopMatch:StopMatch,collapsed:boolean}):JSX.Element{
    ...
    return(<React.Fragment>
        <button className="favorite-toggle" tabIndex={collapsed?-1:0} aria-label='Toggle favorites status for this stop'
            ...
            <span className="add" style={isFavorite(stopMatch)?undefined:{display:'none'}}>Add Stop to Favorites</span>
            <span className="remove" style={isFavorite(stopMatch)?{display:'none'}:undefined}>Remove Stop from Favorites</span>
        </button>
    </React.Fragment>)
}
