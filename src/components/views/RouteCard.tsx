...
({stopDatum, routeId, index}:{stopDatum:StopInterface,routeId:string,index:string}):JSX.Element{
    ...
    try{
        out = (
            ...
            >
                <a href="#" onClick={(e) => {e.preventDefault();search(stopDatum.id.split("_")[1])}} tabIndex="-1">{stopDatum.name}</a>
                {
                    hasVehicleChildren ?
...
export function RouteFavoriteButton({routeMatch,collapsed}:{RouteMatch,boolean}):JSX.Element {
    ...
    return (<React.Fragment>
        ...
        >
            ...
            <span className="add" style={isFavorite(routeMatch)?{display:'none'}:undefined}>Add Route to Favorites</span>
            <span className="remove" style={isFavorite(routeMatch)?undefined:{display:'none'}}>Remove Route from Favorites</span>
        </button>
    </React.Fragment>)
}
